...
contract TodoList {
    ...
    modifier validAddress() {
        require(msg.sender != address(0), "Zero address not alowed");
        _;
    }

    event TodoCreated(string title, Status state);
    event TodoUpdated(string title, Status state);
    ...
    function getTodo(
        ...
        view
        validAddress
        returns (string memory, string memory, Status)
    {
        require(_index < todos.length, "Index is out-of-bound");
